...
drop policy if exists "mvp anon can read courses" on public.courses;
drop policy if exists "mvp anon can insert crawl logs" on public.crawl_logs;
drop policy if exists "mvp anon can read crawl logs" on public.crawl_logs;
drop policy if exists public_read_online_discovery_runs on public.online_discovery_runs;
